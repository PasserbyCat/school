...
def _iou(box_src, boxes_dst):
    ...
    area_intersection = np.maximum(0, (min_x2 - max_x1) + 1) * np.maximum(0, (min_y2 - max_y1) + 1)
...
def _boxes_scale(boxes, model_width, model_height, img_width, img_height):
    ratio_w = model_width / img_width
    ratio_h = model_height / img_height

    # by default KL520, KL720 keep aspect ratio (with padding and resize in firmware)
    ratio_w = ratio_w if ratio_w < ratio_h else ratio_h
    ratio_h = ratio_w

    boxes[..., 2:4] = (boxes[..., 2:4] - boxes[..., 0:2]) / np.array([ratio_w, ratio_h])
    boxes[..., 0:2] = boxes[..., 0:2] / np.array([ratio_w, ratio_h])
    boxes[..., 2:4] += boxes[..., 0:2]
...
def post_process_tiny_yolo_v3(inference_float_node_output_list: List[kp.InferenceFloatNodeOutput],
    ...
    for i in range(len(inference_float_node_output_list)):
        ...
        model_raw_width = inference_float_node_output_list[i].width * MODEL_SHIRNK_RATIO_TYV3[i]
        model_raw_height = inference_float_node_output_list[i].height * MODEL_SHIRNK_RATIO_TYV3[i]

        ratio_w = model_raw_width / inference_float_node_output_list[i].width
        ratio_h = model_raw_height / inference_float_node_output_list[i].height
        ...
        feature_map = _boxes_scale(boxes=feature_map,
                                   model_width=model_raw_width,
                                   model_height=model_raw_height,
    ...
    for idx, candidate_boxes in enumerate(candidate_boxes_list):
        ...
        candidate_boxes_list[idx].x2 = int(image_width - 1) if (
                candidate_boxes_list[idx].x2 + 0.5 > image_width - 1) else int(candidate_boxes_list[idx].x2 + 0.5)
        candidate_boxes_list[idx].y2 = int(image_height - 1) if (
                candidate_boxes_list[idx].y2 + 0.5 > image_height - 1) else int(candidate_boxes_list[idx].y2 + 0.5)
...
def post_process_yolo_v5(inference_float_node_output_list: List[kp.InferenceFloatNodeOutput],
    ...
    for i in range(len(inference_float_node_output_list)):
        ...
        model_raw_width = inference_float_node_output_list[i].width * MODEL_SHIRNK_RATIO_V5[i]
        model_raw_height = inference_float_node_output_list[i].height * MODEL_SHIRNK_RATIO_V5[i]
